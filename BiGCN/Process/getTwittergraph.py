...
def main(obj):
    treePath = os.path.join(cwd, 'BiGCN/data/' + obj + '/data.TD_RvNN.vol_5000.txt')
    ...
    def loadEid(event,id,y):
        ...
        if len(event)>1:
            ...
            np.savez( os.path.join(cwd, 'BiGCN/data/'+obj+'graph/'+id+'.npz'), x=x_x,root=rootfeat,edgeindex=tree,rootindex=rootindex,y=y)
            return None
