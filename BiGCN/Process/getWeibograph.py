...
def main():
    treePath = '/content/BiGCN/data/Weibo/weibotree.txt'
    ...
    labelPath = "/content/BiGCN/data/Weibo/weibo_id_label.txt"
    ...
    def loadEid(event,id,y):
        ...
        if len(event)>1:
            ...
            np.savez('/content/BiGCN/data/Weibograph/'+id+'.npz', x=x_x,root=rootfeat,edgeindex=tree,rootindex=rootindex,y=y)
            return None
        x_word, x_index, tree, rootfeat, rootindex = constructMat(event)
        x_x = getfeature(x_word, x_index)
